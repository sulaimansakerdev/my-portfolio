import Button from "./button";

const AboutMe = () => {
  return (
    <div className="text-center">
      <h2 className="font-extrabold text-[2.8125rem]">About Me</h2>

      <div className="font-semibold text-[0.875rem] bg-gradient-to-r from-sky-400 to-white bg-clip-text text-transparent mb-40">
        Get to know me
      </div>

      <p className="mb-40">
        Hi there! I'm [Your Name], a data scientist specializing in data analytics, predictive
        modeling, natural language processing, machine learning, and AI chatbots. With a passion for
        unraveling insights from complex datasets, I'm dedicated to helping businesses make informed
        decisions and stay ahead in today's data-driven world. <br /> <br /> I bring a blend of
        technical expertise, hands-on experience, and a commitment to clear communication to every
        project. Whether it's uncovering hidden patterns, predicting future trends, or automating
        processes with AI, I'm here to help you harness the full potential of your data. <br />{" "}
        <br /> Let's work together to transform your data into actionable insights that drive real
        results. Get in touch, and let's start unlocking the power of your data today!
      </p>

      <Button variant="rounded">Download Resume</Button>
    </div>
  );
};

export default AboutMe;
